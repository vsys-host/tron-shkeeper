...
class wallet_encryption:
    ...
    def encrypt_db(cls):
        ...
        for row in rows:
            try:
                ...
                query_db2("UPDATE keys SET private = ? WHERE public = ?", (encrypted_private_key, row['public']))
                logger.info(f"{row['public']} account encrypted")
            except (ValueError, tronpy.exceptions.BadKey) as e:
                logger.info(f"{row['public']} is either already encrypted or private key is garbage")
                continue
    ...
    def _fetch_encryption_settings(cls):
        ...
        for symbol in itertools.cycle(['TRX'] + list(config['TOKENS'][config['TRON_NETWORK']])):
            try:
                res = requests.get(f'http://{config["SHKEEPER_HOST"]}/api/v1/{symbol}/decrypt',
                ...
                if res['persistent_status'] == 'enabled':
                    if res['runtime_status'] == 'success':
                        ...
                        logger.info('Wallet encryption is enabled, encryption key is set!')
                        break
                    else:
    ...
    def _validate_encryption_settings(cls):
        ...
        from .db import query_db2
        db_encrypted = None
        if table_exists := query_db2("SELECT * FROM sqlite_master WHERE type='table' AND name='keys'", one=True):
        ...
        if db_encrypted is None:
            logger.info('Wallet is not initialized yet. Skipping encryption validation.')
            return None

        if cls.encryption is True and db_encrypted is False and config.get('FORCE_WALLET_ENCRYPTION'):
            logger.info('DB is unencrypted, force wallet encryption is requested by env')
            cls.encrypt_db()
            return True
            ...
            if cls.encryption is db_encrypted:
                ...
                raise EncryptionModeMismatch(f"Shkeeper requested encryption={cls.encryption} but "
