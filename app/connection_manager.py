...
class ConnectionManager:
    ...
    def __init__(self) -> None:
        if config['MULTISERVER_CONFIG_JSON']:
            self.servers = json.loads(config['MULTISERVER_CONFIG_JSON'])
        elif config['FULLNODE_URL']:
            url = urlparse(config['FULLNODE_URL'])
            new_netloc = f"{config['TRON_NODE_USERNAME']}:{config['TRON_NODE_PASSWORD']}@{url.netloc}"
            url_with_creds = url._replace(netloc=new_netloc).geturl()
            self.servers = [{"name": url.hostname, "url": url_with_creds}]
        else:
            raise Exception('No FULLNODE_URL or MULTISERVER_CONFIG_JSON env variables are set!')
    ...
    def get_current_server_id(self):
        row = query_db2('SELECT value FROM settings WHERE name = "current_server_id"', one=True)
        if row:
    ...
    def set_current_server_id(self, server_id):
        query_db2('UPDATE settings SET value = ? WHERE name = "current_server_id"', (server_id,))
    ...
    def get_servers_status(self):
        ...
        for server_id, server in enumerate(self.servers):
            try:
                ...
                node_info["block"] = int([j for i in node_info["block"].split(',') for j in i.split(':')][1])

                # last block info
                resp = requests.post(f'{server["url"]}/wallet/getblockbynum', json={"num": node_info["block"]})
                resp.raise_for_status()
                block = resp.json()
                node_info["block_ts"] = block['block_header']['raw_data']['timestamp'] // 1000
                ...
                status = {'id': server_id, 'is_active':  self.get_current_server_id() == server_id,
                           **server, 'status': 'success', 'node_info': node_info}
